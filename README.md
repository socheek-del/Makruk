...
the in-app lessons and [`docs/rules.md`](docs/rules.md) explain them step by step.
...
flowchart LR
  ...
  UI -- "WebSocket" --> Room
  UI -- "WebSocket" --> Match
  Room --> Engine2["@makruk/engine\n(validates every move)"]
```

| Package | What it does |
|---|---|
| [`packages/engine`](packages/engine) | Pure TypeScript rules engine — move generation, counting rules, FEN, perft; cross-checked against Fairy-Stockfish |
...
| [`apps/web`](apps/web) | The React PWA: board, lessons, pass-and-play and the online client |
| [`apps/worker`](apps/worker) | Cloudflare Worker with Durable Objects for game rooms, clocks and matchmaking |
...
```bash
git clone https://github.com/socheek-del/Makruk.git
cd Makruk
nvm use          # Node 22
...
- 🐛 [Report a bug or suggest an idea](https://github.com/socheek-del/Makruk/issues/new)
