/**
 * sit-010: per-page title, description, canonical URL and language alternates for the Sittuyin site. The
 * head-tag writing and language URLs are shared (@chaturanga/game-shell); pages and languages are Sittuyin's.
 */
import {
...
export const SEO_PAGES: readonly SeoPage[] = ['home', 'computer', 'online', 'room', 'local', 'learn', 'about', 'settings'];
...
export function pageSeo(pathname: string): PageSeo {
  ...
  if (path === '/learn' || path.startsWith('/learn/')) return { page: 'learn', path: '/learn', index: path === '/learn' };
...
/** Burmese pages live at the plain URL (the default language); English at `?lang=en`. */
export function localizedUrl(path: string, language: Language): string {
  return productUrl(PRODUCT, SITE_URL, path, language);
}

/** Reads `?lang=my|en` from a URL search string. */
