...
test('settings offer 6 board themes and 2 piece sets with previews; the choice applies and persists (theme-002)', async ({ page }) => {
  await page.goto('/settings');
  await expect(page.locator('[data-board-theme]')).toHaveCount(6);
  await expect(page.locator('[data-piece-set]')).toHaveCount(2);
  await expect(page.locator('[data-piece-set="classic"] svg[data-set="classic"]')).toHaveCount(7);
  await expect(page.locator('[data-piece-set="flat"] svg[data-set="flat"]')).toHaveCount(7);
